# Codex / Claude Code Coding Agent Project Guide

## User Context

This project is a collaborative test for a team of non-developers learning to work with coding agents in a shared GitHub environment. This document was adapted from instructions used by one team member for an indvidiual project.

In addition to the instructions provided in this file, part of this project will adding instructions for the agent to assist team members in managing the shared GitHub environment. Accordingly, you should offer suggested edits where appropriate.

The users are not developers, coders, or software engineers.

The users have varying degrees of basic but functional understanding of GitHub and Codex.

The users may use either a local Codex or a Claude Code web/cloud workflow.

Do not assume the user is operating a traditional terminal workflow unless the user explicitly says so.

The coding agent should perform routine local development, Git inspection, file editing, testing, committing, and pushing when possible.

The coding agent should ask the user to manually run terminal commands only when there is a specific blocker that it cannot handle itself.
...
1. What the coding agent will do.
...
The coding agent should run safe commands itself from the workspace whenever possible.

Examples of commands the coding agent should usually run:
...
The coding agent  may need user approval, authentication, or manual action for:
...
The coding agent  must be cautious and explicit before using:
...
If the coding agent  cannot run a command itself, it must explain:

1. The exact command that needs to run.
2. Why the agent cannot run it.
...
This project may be stored locally inside a OneDrive-synced folder.
...
The coding agent  should handle this as follows:
...
If a stale Git lock file exists after a failed Git command, The coding agent  may remove only the specific stale lock file after confirming it is safe.
...
Known working path for user Casey:
...
When running npm scripts, the agent may need to prefix `PATH` so scripts resolve to portable Node instead of the Codex desktop bundled Node:
...
For verification, the agent may build to a temporary output folder instead of `dist`:
...
If `gh` is unavailable, the agent should still use local Git for branch, commit, and push operations when possible, then provide the direct GitHub pull request URL:

```text
https://github.com/caseywalrath/roadway_cost_estimator/pull/new/[branch-name]
```

Browser automation through the in-app browser may fail because the Node REPL reports local Node access errors. If that happens, the agent should use source inspection, local HTTP checks, and user visual confirmation rather than claiming browser automation verification occurred.
...
The agent should handle this as follows:
...
In that case, The agent may use an unrelated-history merge only after confirming the remote contents are understood and nothing will be overwritten.

The agent should prefer preserving both local and remote files.
...
1. Review codex.md.
2. Review architecture.md if it exists.
...
Before committing, the agent should run the most relevant local check available.
...
The agent should give the user the local URL when a local server is running.
...
After local testing is complete, the agent should summarize changed files before committing.
...
After committing, the agent may push the branch to GitHub if authentication and permissions allow it.

If pushing requires approval or login, the agent must explain the blocker clearly.
...
Before closing a session, the agent should:
...
The agent must review `architecture_overview.md` before new session plans or code changes.

The agent should update `architecture_overview.md` when architecture, file structure, deployment behavior, data flow, or major app behavior changes.
